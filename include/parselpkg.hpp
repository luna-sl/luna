...
class Array : public Value
{
  ...
  public:
	Array(std::vector<std::string> content) : super(Type::ARRAY), contents(content)
	{
	}
...
class Function : public Value
{
  ...
  public:
	Function(std::vector<std::string> content) : super(Type::FUNCTION), contents(content)
	{
	}
	std::vector<std::string> getContents()
	{
		...
		while (stream.get(currentTok))
		{
			switch (currentTok)
			{
				...
				case 's': {
					...
					result.insert({strname, Lpkg::String(strval)});
					break;
				}
				case 'a': {
					...
					result.insert({arrname, Lpkg::Array(arrvals)});
					break;
				}
				case 'f': {
					...
					result.insert({funcname, Lpkg::Function(funccmds)});
					break;
				}
			}
		}
