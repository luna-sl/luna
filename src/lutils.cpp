...
void privEsc()
{
	if (getuid() != 0)
	{
		...
		log(LogLevel::WARN, "missing permissions. attempting to rerun as {}root{}...", color(200, 145, 145) + bold(),
			/*"root"*/ colorTerminate());
...
int copy_data(struct archive *ar, struct archive *aw)
{
	...
	for (;;)
	{
		...
		if (r != ARCHIVE_OK)
		{
			log(LogLevel::WARN, "Extraction warning @ {}, {}", "archive_write_data_block()", archive_error_string(aw));
			return (r);
		}
	}
}
void extract(const char *filename, int flags, const char *destdir)
{
	...
	if ((r = archive_read_open_filename(a, filename, 10240)))
		log(LogLevel::FATAL, "Extraction fail @ {}, {}, {}", "archive_read_open_filename()", archive_error_string(a),
			r);
	for (;;)
	{
		...
		if (r == ARCHIVE_EOF)
			break;
		if (r != ARCHIVE_OK)
			log(LogLevel::FATAL, "Extraction fail @ {}, {}, {}", "archive_read_next_header()", archive_error_string(a),
				1);
		char* str = (char*)malloc(4096*sizeof(char));
		strcpy(str, destdir);
		log(LogLevel::DEBUG, "{}", archive_entry_pathname(entry));
		archive_entry_set_pathname(entry, strcat(str, archive_entry_pathname(entry)));
		r = archive_write_header(ext, entry);
		if (r != ARCHIVE_OK)
			log(LogLevel::WARN, "Extraction warning @ {}, {}", "archive_write_header()", archive_error_string(ext));
		else
		{
			copy_data(a, ext);
			r = archive_write_finish_entry(ext);
			if (r != ARCHIVE_OK)
				log(LogLevel::FATAL, "Extraction fail @ {}, {}, {}", "archive_write_finish_entry()",
