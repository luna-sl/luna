...
void updateRepos() {
	...
	std::function<void()> func = [&]() {
		...
		if (reposListFile.is_open()) {
			...
			std::vector<std::string> reposList =
			    splitstr(tmp, "\n");
			for (int i = 0; i < reposList.size(); ++i) {
				L.setProgress(
				    format("{}/{}", i + 1, reposList.size()));
				cpr::Response r =
				    cpr::Get(cpr::Url{reposList.at(i)});
				log(
				    r.error.code != cpr::ErrorCode::OK,
				    [&L]() { L.fail(); }, LogLevel::FATAL,
				    "failed to get {} because: {}",
				    r.url.str().substr(
					r.url.str().find_last_of("/") + 1),
				    r.error.message);
				log(
				    r.status_code != 200, [&L]() { L.fail(); },
				    LogLevel::FATAL,
				    "failed to get {} because request failed "
				    "with response code: {}",
				    r.url.str().substr(
					r.url.str().find_last_of("/") + 1),
				    r.status_code);
				std::ofstream repoOut(format(
